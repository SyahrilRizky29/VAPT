...
// untuk debugging di console (hapus sebelum publish)
...
async function handleCheckout(e) {
  ...
  const cartItems = cartService.getCart(); // [{productId, quantity}, ...]
  if (!cartItems || cartItems.length === 0) {
    showAlert('Keranjang kosong', 'error');
    return;
  }

  // For simplicity: assume single-product checkout (adapt if multiple)
  ...
  try {
    // 1) request server to create signed cart (server computes serverTotal + signature)
 const user = authService.getCurrentUser();
const username = (user && (user.username || user.email)) || null;
if (!username) {
  showAlert('Silakan login terlebih dahulu', 'error');
  openAuthModal();
  return;
...
fetch('/create_cart', {
  ...
  body: JSON.stringify(payload)
    ...
    if (!createJson.ok) throw new Error(createJson.error || 'Gagal membuat cart');

    let cart = createJson.cart;
    // At this point attacker can intercept response OR intercept next request and tamper 'cart' fields
    // 2) send checkout request with the cart object (this is what Burp should intercept)
    ...
    if (!checkoutJson.ok) throw new Error(checkoutJson.error || 'Checkout gagal');

    // on success: clear cart (client-side) and navigate to orders
    cartService.clearCart();
    updateCartBadge();
    showAlert('Pesanan berhasil dibuat (server side)', 'success');
    navigateTo('orders');
  } catch (err) {
    showAlert(err.message || 'Terjadi kesalahan saat checkout', 'error');
  }
}
